...
Looks like there is a high correlation between white people and voting turnout. There is almost zero correlation between those who have a bachelor's or higher and voting. Income and the percentage of white people are the least correlated statistics. The most linear relationships to me seem to be income and percentage unemployed.
...
The residuals are fairly small, meaning that the predictions were pretty close to the actual results all around. The p-value is also really small, so we can reject the null hypothesis, and the F-statistic is high, so we can consider the model significant. It looks like the median age, the a lot of white people, and the amount of income you have are the most statistically significant independent variables in the data and if you are or have a lot of any of those things then you are more likely to vote. I'm not entirely sure i understand the basic premsie but my guess is that whe you get to your forties and such you are more likely to vote.Population, education and employment are all in the negatives and don't really hold a candle to it.The multiple R squared was at 0.8539 and the r square adjusted was 0.8023. So the model was certinaly significant.
...
I took the three highest performing independent variables from the model, and I'm a little confused why what happened happened. I mean a little. It all added up to 80% for the adjusted squared for the full model, but the multiple R-squared was pretty low, and I guess that didn't compute in my head. I put in pct_unemployed next, and it didn't change the graph or the models at all. But the multiple and adjusted R were now in the 50's and i think the f-statisitc jumped from a 16 or 20 to a 26. Also when i plugged in the coefficients of. income, median age, and pct_whiteFor some reason, it only showed the first coefficients data. 
...
# Model 4
model4 <- lm(turnout ~pct_unemployed, data = md_turnout)
summary(model4)
...
First off this sort of model is incredibly helpful in testing multiple independent variables to see the correlation between the two and how the measure to the dependent variable. Below is my attempt at a nut-graph

"Why arent minorities coming out to vote in the same way old white rich people are? According to data recetnly analyzed by the Cassett Institute of Analyzing, (the least fun CIA), the most significant varaibles in voter turnout come down to income, how white you are, and how old you are. A mixture of the three being, middle aged, rich and white. The questions remains, why arent those who have the least and most of their lives ahead of them coming to the polls."
