...
```{r}
murders_per_month <-read_csv("https://raw.githubusercontent.com/dwillis/jour405/refs/heads/main/data/pg_murder_temps.csv")
...
```{r}
murders_per_month |> summarize(corellation = cor(murders,month,temperature, method= "pearson"))
```
...
murders_per_month |>
  ggplot()+
  geom_point(aes(x=month,y=temperature))
```
...
murders_per_month |>
  ...
  geom_text(aes(x=month,y=temperature)size.unit = 5)
...
I had a rough time with this one and will need to go over it with matt
