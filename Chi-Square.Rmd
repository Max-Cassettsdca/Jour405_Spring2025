...
The amount of Five Star ratings is almost half as much as the other ratings. For some reason, it was expected that there would be more than a frequency of 145 meaning that most movies are good instead of all movies being judged against each other creating an equal percentage of reviews. 
...
If the calculated chi-square statistic (9.9521)is greater than the right-tail critical value (9.487729), we reject the null hypothesis and conclude that the observed frequencies are significantly different from the expected frequencies.
...
We reject the null hypothesis, meaning that the observed frequencies are significantly different from the expected frequencies, which is the alternative hypothesis. The reason that it is still significant despite being so close is because it's going off of proportion instead of points.
